...
app resources folder ResourcesFolder/cache folder, to avoid re-scannings. 
  ...
  - `data/BROWSER-LOCAL-MODS.json`: the merged local-mods cache (Everest scan output + `LocalMods_Map_ModId_To_MaddiesInfo` keyed by mod id) that `LocalMods`/`Construct_LocalMods` reads and writes via `Storage_JsonFileAdapter`. Two top-level keys: `localmods_allInstalled` (raw per-mod Everest metadata, keyed by mod name) and `LocalMods_Map_ModId_To_MaddiesInfo` (MaddiesAPI enrichment, keyed by mod id). This file lives directly under `data/`, NOT under `data/cache/`.
