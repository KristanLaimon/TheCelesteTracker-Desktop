...
export class NeutralinoFileSystem implements IFileSystem {
		...
		if (NeutralinoFileSystem.ResourcesFolderPath_Backend === null) {
			...
			const resourcesFolderExists = await (async () => {
				try {
					...
					coreLogger.error("NeutralinoFileSystem:", e);
					return false;
				}
			})();
			if (!resourcesFolderExists) {
				await filesystem.createDirectory(resourcesFolderPath);
				coreLogger.info("NeutralinoFileSystem: Resources folder not found, creating empty directory at exe's path.");
			}
			...
			if (Object.keys(res).length > 0) {
				if (res[NeutralinoFileSystem.ResourcesFolderPath_Frontend]) {
					coreLogger.trace("NeutralinoFileSystem: Already Mounted Paths.. Skipping the following remounts", res);
				} else {
					await server.mount(NeutralinoFileSystem.ResourcesFolderPath_Frontend, NeutralinoFileSystem.ResourcesFolderPath_Backend);
					coreLogger.trace("NeutralinoFileSystem: Mounted and loaded local folder:", NeutralinoFileSystem.ResourcesFolderPath_Backend);
				}
			} else {
				await server.mount(NeutralinoFileSystem.ResourcesFolderPath_Frontend, NeutralinoFileSystem.ResourcesFolderPath_Backend);
				coreLogger.trace("NeutralinoFileSystem: Mounted and loaded local folder:", NeutralinoFileSystem.ResourcesFolderPath_Backend);
	...
	public async exists(path: string): Promise<boolean> {
		try {
			...
			coreLogger.error("NeutralinoFileSystem.exists error:", e);
			return false;
		}
	}
