...
<div class="fixed inset-0 bg-background/80 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
  <div class="bg-card w-full max-w-md rounded-2xl border border-border shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
    <div class="p-6 border-b border-border flex justify-between items-center bg-muted/30">
      <div>
        <h2 class="text-xl font-black tracking-tight">Edit Run</h2>
        <p class="text-[10px] text-muted-foreground uppercase font-bold tracking-widest mt-1">ID: #{run.id}</p>
      </div>
    ...
    <div class="p-8 space-y-8">
      <div class="space-y-3">
        <label for="deaths" class="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground ml-1">Total Deaths</label>
        <div class="relative group">
          <Ghost class="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-muted-foreground group-focus-within:text-primary transition-colors" />
          <input
            id="deaths"
            type="number"
            bind:value={deaths}
      ...
      <div class="space-y-3">
        <label for="strawberries" class="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground ml-1">Strawberries</label>
        <div class="relative group">
          <Zap class="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-muted-foreground group-focus-within:text-primary transition-colors" />
          <input
            id="strawberries"
            type="number"
