<script lang="ts">
  ...
  interface Props {
     ...
     * @default {}
     */
    registry?: Record<string, Component<Record<string, any>>>;
    /**
  ...
  function observeSize(nodeEl: HTMLElement, node: CanvasNodeData) {
    const observer = new ResizeObserver((entries) => {
      for (const entry of entries) {
        const width =
          entry.borderBoxSize?.[0]?.inlineSize ?? nodeEl.offsetWidth;
        const height =
          entry.borderBoxSize?.[0]?.blockSize ?? nodeEl.offsetHeight;
