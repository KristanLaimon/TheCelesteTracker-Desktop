<script lang="ts">
  ...
  import { GetCurrentAppStatus, ScanModAssets } from "../../wailsjs/go/app/App";
  ...
  onMount(async () => {
    try {
      appStatus = await GetCurrentAppStatus();
      if (appStatus.is_mod_db_initialized) {
        // Trigger asset scan in background
        ScanModAssets().catch(console.error);
      }
    } catch (e) {
  ...
  const openModLink = async () => {
    await BrowserOpenURL("https://github.com/KristanLaimon/TheCelesteTracker-Mod");
  };
</script>
